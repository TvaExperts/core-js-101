...
function getPowerFunction(exponent) {
  const constExponent = exponent;
  return function aa(value) {
    return value ** constExponent;
  };
}
...
function getPolynom(...args) {
  ...
  return (x) => {
    ...
    return koefs.reduce(
      (sum, koef, i) => sum + koef * x ** (koefs.length - i - 1),
      0,
    );
  };
}
...
function retry(func, attempts) {
  let curAtempt = 0;
  const maxAtemts = attempts;
  const fn = func;
  return () => {
    while (curAtempt <= maxAtemts) {
      try {
        return fn();
      } catch (e) {
        curAtempt += 1;
...
function logger(func, logFunc) {
  ...
  return (...args) => {
    ...
    logFn(`${mainFn.name}(${argsStr}) starts`);
    const res = mainFn(...args);
    logFn(`${mainFn.name}(${argsStr}) ends`);
    return res;
  };
}

...
function isCreditCardNumber(ccn) {
  ...
  const modifiedArray = numbersArray
    ...
    .map((item, i) => {
      ...
      const itemX2 = item * 2;
      return itemX2 > 9 ? itemX2 - 9 : itemX2;
    });
  const sumArray = modifiedArray.reduce((sum, item) => sum + item);
...
function isBracketsBalanced(str) {
  ...
  let oldLength = 0;
  let curStr = str;
  while (oldLength !== curStr.length) {
    oldLength = curStr.length;
    for (let i = 0; i < bracketsArr.length; i += 1) {
      curStr = curStr.replaceAll(bracketsArr[i], '');
    }
  }
